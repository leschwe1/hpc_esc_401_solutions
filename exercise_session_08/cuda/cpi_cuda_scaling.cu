...
int main(int argc, char *argv[]) {
    ...
    // i modified the main function  so that i can pass the two vars BLOCK and THREAD in cmd line
    // so now i can run /cpi_cuda_scaling 120 64 for example
    if (argc < 3) {
        printf("Usage: %s <NUM_BLOCK> <NUM_THREAD>\n", argv[0]);
        return 1;
    }
    int NUM_BLOCK  = atoi(argv[1]);  // now as input arg
    int NUM_THREAD = atoi(argv[2]);  // now as input arg
