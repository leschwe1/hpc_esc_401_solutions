...
From the below output we get:

Numbers for the mapping process
- Launched map tasks=7 
- Map input records=35371
- Map output records=538568

Numbers for the reduction process:
- Launched reduce tasks=1 (= reduce operations)
...
So the output was stored in the hdfs (Hadoop's FS) 
we ghet two files:
  hdfs dfs -ls output
  Found 2 items
  -rw-r--r--   3 ubuntu supergroup          0 2025-12-18 10:53 output/_SUCCESS
  -rw-r--r--   3 ubuntu supergroup     642476 2025-12-18 10:53 output/part-00000

as output/_SUCCESS is empty, we focus on part-00000

I copied it to the local storgae with
...
On http://localhost:8088 i cannot find any output, but I can see that my process is under "finished". So everything worked. 
The tracking UI and the link to the node do not work anymore as the node was shutdown after the process has ended

But on http://localhost:9870
I find the filesystem.
And i can browse it.
Ending up under the url http://localhost:9870/explorer.html#/user/ubuntu/output (analogous to the filepath)
I get to the same output files as mentioned above when accessing them locally in the console. But a download from there was not possible?


# Output of running pymapred.sh
